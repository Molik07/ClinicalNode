    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1fr] gap-8 lg:gap-12 mb-12">
        ...
        <div className="sm:col-span-2 lg:col-span-1">
          ...
          <p className="font-inter text-[11px] tracking-[0.06em] leading-8 text-muted-dark max-w-[220px]">
            ...
            <div className="flex flex-col gap-2.5">
              {col.links.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  className="font-inter text-[11px] tracking-[0.06em] text-[rgba(184,204,176,0.48)] uppercase transition-colors duration-150 hover:text-text-on-dark"
                >
                  {link.label}
                </Link>
              ))}
      ...
      <div className="border-t-[0.5px] border-line-dark pt-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="flex flex-wrap gap-3 sm:gap-[18px]">
          {['Privacy Policy', 'Terms of Use', 'Accessibility'].map((text) => (
            <span
              key={text}
              className="font-inter text-[9.5px] tracking-[0.08em] text-[rgba(184,204,176,0.25)] uppercase transition-colors duration-150 hover:text-[rgba(184,204,176,0.5)] cursor-pointer"
            >
              {text}
            </span>
          ))}
